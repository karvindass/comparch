...
void MEM_stage() {
    ...
    we0, we1,
    dcache_r,
    mem_ir_11, mem_ir_10, mem_ir_9,
    mem_n, mem_z, mem_p,
    mem_ben;
    
    mem_address_0 = PS.MEM_ADDRESS%2;
    
    /* Align Logic */
    if (PS.MEM_CS[MEM_DATA_SIZE]==0)
    {
        if (mem_address_0==0) { dcache_data_i = PS.MEM_ALU_RESULT; }
        else { dcache_data_i = Low16bits( (PS.MEM_ALU_RESULT<<8) | (PS.MEM_ALU_RESULT & 0x00FF) ); }
    }
    else { dcache_data_i = PS.MEM_ALU_RESULT; }
    
    /* WE Logic */
    if (PS.MEM_CS[MEM_DCACHE_RW]==1)    /* Write */
    {
        if (PS.MEM_CS[MEM_DATA_SIZE]==0)    /* Byte */
        {
            if (mem_address_0==0) { we0 = 1; we1 = 0; } /* Even Address */
            else { we0 = 0; we1 = 1; }  /* Odd Address */
        }
        else { we0 = 1; we1 = 1; }  /* Word */
        
    }
    else { we0 = 0; we1 = 0; }  /* Read */
    
    v_dcache_en = (PS.MEM_V) && (PS.MEM_CS[MEM_DCACHE_EN]);
    
    /* D Cache Access */
    if (v_dcache_en==1)
        dcache_access(PS.MEM_ADDRESS, &dcache_data_o, dcache_data_i, &dcache_r, we0, we1);
    else
        dcache_r = 1;
    ...
    /* BR Logic */
    if (PS.MEM_V==0) mem_pcmux = 0;
    else
    {
        if ( PS.MEM_CS[MEM_BR_OP]==1 )
        {
            mem_ir_11 = (PS.MEM_IR>>11)%2;
            mem_ir_10 = (PS.MEM_IR>>10)%2;
            mem_ir_9 = (PS.MEM_IR>>9)%2;
            mem_n = (PS.MEM_CC>>2)%2;
            mem_z = (PS.MEM_CC>>1)%2;
            mem_p = PS.MEM_CC%2;
            mem_ben = (mem_ir_11 && mem_n) || (mem_ir_10 && mem_z) || (mem_ir_9 && mem_p);
            if (mem_ben==1) mem_pcmux = 1;
        }
    ...
    /* Write-to-Latch Logic */
    if (v_dcache_en == 0) trap_pc = 0;
    else
    {
        if (PS.MEM_CS[MEM_DATA_SIZE]==1)    /* Word */
        {
            trap_pc = dcache_data_o;
        }
        else    /* Byte */
        {
            if (mem_address_0==0)   /* Even Address */
                trap_pc = Low16bits( dcache_data_o & 0x00FF );
            else    /* Odd Adddress */
                trap_pc = Low16bits( (dcache_data_o>>8) & 0x00FF);
            
            if( (trap_pc>>7)%2==1 ) trap_pc = Low16bits( trap_pc | 0xFF00 );    /* Sign Extended */
        }
    }
...
void AGEX_stage() {
    ...
    switch (PS.AGEX_CS[AGEX_ADDRESSMUX])
    {
        case 0: { mem_address = Low16bits(( PS.AGEX_IR & 0x00FF )<<1); } break;
        
        case 1:
        {
            switch ( PS.AGEX_CS[AGEX_ADDR1MUX] )
            {
                case 0: { A = PS.AGEX_NPC; } break;
                case 1: { A = PS.AGEX_SR1; } break;
                default: break;
            }
            
            agex_addr2mux = ( PS.AGEX_CS[AGEX_ADDR2MUX1]<<1 ) + ( PS.AGEX_CS[AGEX_ADDR2MUX0] );
            ...
            switch ( agex_addr2mux )
            {
                ...
                {
                    B = PS.AGEX_IR%64;
                    if (B>31) B = B | 0xFFFFFFC0;
                }
                    break;
                case 2:
                {
                    B = PS.AGEX_IR%512;
                    if (B>255) B = B | 0xFFFFFE00;
                }
                    break;
                case 3:
                {
                    B = PS.AGEX_IR%2048;
                    if (B>1023) B = B | 0xFFFFF800;
                }
                    break;
                default: break;
            }
            if ( PS.AGEX_CS[AGEX_LSHF1] == 1 ) B = (B<<1);
            
            mem_address = Low16bits( A + B );
        }
            break;
    ...
    /* MEM.ALU.RESULT Generation Logic */
    switch ( PS.AGEX_CS[AGEX_ALU_RESULTMUX] )
    {
        case 0:
        {
            shfmux = (PS.AGEX_IR>>4)%4;
            amount4 = PS.AGEX_IR%16;
            switch (shfmux)
            {
                case 0: { mem_alu_result = Low16bits( PS.AGEX_SR1<<amount4 ); } break;
                case 1: { mem_alu_result = Low16bits( PS.AGEX_SR1>>amount4 ); } break;
                case 3:
                {
                    mem_alu_result = Low16bits( PS.AGEX_SR1>>amount4 );
                    if( PS.AGEX_SR1 > 32767 )
                    {
                        for( i=0; i<amount4; i++ ) sum = sum + ( 1<<(15-i));
                        mem_alu_result = Low16bits( mem_alu_result + sum );
                    }
        ...
        case 1:
        {
            ...
            switch ( PS.AGEX_CS[AGEX_SR2MUX] )
            {
                case 0: { B = PS.AGEX_SR2; } break;
                case 1:
                {
                    B = PS.AGEX_IR%32;
            ...
            switch ( aluk )
            {
                case 0: { mem_alu_result = Low16bits( A + B ); break; }
                case 1:	{ mem_alu_result = Low16bits( A & B ); break; }
                case 2: { mem_alu_result = Low16bits( A ^ B ); break; }
                case 3: { mem_alu_result = Low16bits( B ); break; }
                default: break;
            }
        }
            break;
...
void DE_stage() {
    ...
    CONTROL_STORE_ADDRESS = (((PS.DE_IR>>11)%32)<<1) + (PS.DE_IR>>5)%2;
    
    /* V.DE.BR.STALL Generation Logic */
    v_de_br_stall = PS.DE_V && CONTROL_STORE[CONTROL_STORE_ADDRESS][BR_STALL];
    
    /* REG FILE Read & Write */
    sr1_id = (PS.DE_IR>>6)%8;
    agex_sr1 = REGS[sr1_id];
    switch ( (PS.DE_IR>>13)%2 )
    {
        case 0: { sr2_id = PS.DE_IR%8; } break;
        case 1: { sr2_id = (PS.DE_IR>>9)%8; } break;
        default: break;
    }
    agex_sr2 = REGS[sr2_id];
    if ( v_sr_ld_reg == 1 ) REGS[sr_drid] = Low16bits( sr_reg_data );
    
    /* Condition Code Read & Write */
    agex_n = N;
    agex_z = Z;
    agex_p = P;
    if ( v_sr_ld_cc == 1 )
    ...
    /* AGEX.DRID Generation Logic */
    if ( CONTROL_STORE[CONTROL_STORE_ADDRESS][DRMUX] == 0 ) de_agex_drid = (PS.DE_IR>>9)%8;
    else de_agex_drid = 7;
    
    /* Dependency Check Logic */
    sr1_needed = CONTROL_STORE[CONTROL_STORE_ADDRESS][SR1_NEEDED];
    sr2_needed = CONTROL_STORE[CONTROL_STORE_ADDRESS][SR2_NEEDED];
    de_br_op = CONTROL_STORE[CONTROL_STORE_ADDRESS][BR_OP];
    if ( PS.DE_V == 1 )
    {
    ...
    if (LD_AGEX) {
        ...
        NEW_PS.AGEX_CC = (agex_n<<2) + (agex_z<<1) + agex_p;
        NEW_PS.AGEX_DRID = de_agex_drid;
...
void FETCH_stage() {
    ...
    /* DE.NPC Generation Logic */
    de_npc = Low16bits( PC + 2 );
    
    /* I-Cache Access */
    icache_access(PC, &de_ir, &icache_r);
    
    /* LD.PC Generation Logic */
    /* Version 1
    if ( dep_stall || mem_stall || (icache_r==0) || v_de_br_stall || v_agex_br_stall || v_mem_br_stall )*/
    /* Version 2
    if ( dep_stall || mem_stall || (icache_r==0) || v_de_br_stall || v_agex_br_stall )*/
    /* Version 3
    if ( dep_stall || mem_stall || v_de_br_stall || v_agex_br_stall || (icache_r==0 && v_mem_br_stall ==0))
        ld_pc = 0;
    else
        ld_pc = 1;*/
    /* Version 4 */
    ...
    if ( ld_pc == 1 )
        switch ( mem_pcmux )
        {
            case 0: { PC = Low16bits( PC+2 ); } break;
            case 1: { PC = Low16bits( target_pc ); } break;
            case 2: { PC = Low16bits( trap_pc ); } break;
            default: break;
        }
    ...
    if ( ld_pc == 0 ) de_v = 0; */
    /* Version 2 */
    if ( ld_pc == 0 || v_mem_br_stall ) de_v = 0;
    /* Version 3
